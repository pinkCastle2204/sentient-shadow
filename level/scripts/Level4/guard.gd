...
@export var is_talker: bool = false  # true for guard/NPC — talks before fighting instead of attacking on sight
...
@onready var collision_shape_2db: CollisionShape2D = $CollisionShape2D
...
# Default x position of the hurt box when facing right
...
func set_facing(direction: float) -> void:
	...
	collision_shape_2db.position.x = hit_box_shift_x if facing > 0 else -hit_box_shift_x
